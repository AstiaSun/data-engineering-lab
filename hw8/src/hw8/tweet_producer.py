...
def _message_sent_callback(error: Any, message: KafkaMessage):
    if error is not None:
        ...
        logger.debug(
            f"Message delivered to {message.topic()} [{message.partition()}] @ offset {message.offset()}"
        )
...
class TweetProducer:
    ...
    def handle_shutdown(self, signum: int, _: Any):
        ...
        if not_delivered_messages:
            logger.error(
                f"Failed to deliver messages to kafka: number={not_delivered_messages}"
            )
    ...
    def send_tweets_from_steam(
        self, stream: Iterable[Tweet], *, rps: int = 15, rate_limit: int = 20
    ):
        while True:
            if not self._active:
                return
